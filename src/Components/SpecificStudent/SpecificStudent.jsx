...
export default function SpecificStudent() {
  ...
  function getSingleStudent() {
    axios
      .get(
        `https://registration-production-7fcd.up.railway.app/api/v2/students/${studentID}`
      )
      .then((response) => {
  ...
  const topFive = async () => {
    try {
      const response = await axios.get(
        `https://registration-production-7fcd.up.railway.app/api/v2/students/${groupID}/Top5`
      );
          ...
          <div className="py-1 px-1">
            <div className="row mt-1">
              <div className="col-md-6">
                <div
                  className={`${Style.giro} p-4 rounded-2`}
                  style={{ direction: "rtl" }}
                >
                  <div className="d-flex align-items-center bg-white p-2 rounded-2">
                    <div className="mx-5">
                      <i
                        className={`${Style.goldi} fa-solid fa-medal fs-2`}
                      ></i>
                    </div>
                    <div>
                  ...
                  <div className="d-flex align-items-center mt-2 p-2">
                    <div className="mx-5">
                      <i
                        className={`${Style.silveri} fa-solid fa-medal fs-2`}
                      ></i>
                    </div>
                    <div>
                  ...
                  <div className="d-flex align-items-center mt-2 p-2">
                    <div className="mx-5">
                      <i
                        className={`${Style.bronzi} fa-solid fa-medal fs-2`}
                      ></i>
                    </div>
                    <div>
                ...
                <div className={`${Style.bgNav} p-4 rounded-2 mt-3`}>
                  <h3 className="text-center text-white fw-bolder">
                    نتيجتك : {studentData?.averageGrades}{" "}
                  </h3>
                </div>
              </div>
              <div className="col-md-6 mt-md-0 mt-3">
                  ...
                  {studentData && (
                    <>
                      ...
                      <h3 className="mt-3 p-2">
                        رقم الطالب : {studentData.phoneNumber}
                      </h3>
                      <h3 className="mt-3 p-2">
                        رقم الوالد : {studentData.guardianPhoneNumber}
                      </h3>
                      <h3 className="mt-3 p-2">
                        كود الطالب : {studentData.studentCode}
                      </h3>
                      <h3 className="mt-3 p-2">
                        سعر الحصة : {studentData.price}
                      </h3>
                      <h3 className="mt-3 p-2">
                        سعر الكتب : {studentData.bookPrice}
                      </h3>
                      <h3 className="mt-3 p-2">
                        ملاحظات : {studentData.description}
                      </h3>
                    </>
                  )}
                </div>
                <div className="mt-3">
                  <button
                    className={`${Style.cordi} py-2 rounded-2 fw-bolder w-100`}
                  ...
                  <div className="d-flex mt-2">
                    <button
                      className={`${Style.cordi} me-2 py-2 rounded-2 fw-bolder w-100`}
                      onClick={handleHomeWorkGradeClick}
                    >
                      الواجب المنزلي
                    </button>
                    <button
                      className={`${Style.cordi} py-2 rounded-2 fw-bolder w-100`}
                      onClick={handleExamGradeClick}
                    >
                      نتيجة الاختبارات
                    </button>
                  </div>
