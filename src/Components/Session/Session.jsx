...
export default function Session() {
  ...
  useEffect(() => {
    axios
      .get(
        `https://registration-production-7fcd.up.railway.app/api/v2/sessions/${ID}`
      )
      .then((response) => {
  ...
  const deleteSession = async (id) => {
    try {
      await axios.delete(
        `https://registration-production-7fcd.up.railway.app/api/v2/sessions/${ID}/${id}`
      );
        ...
        <div className="container-fluid">
          <div className="row mb-4">
            ...
            <div className="col-12 d-flex flex-column flex-md-row justify-content-between align-items-center">
              <Link
                onClick={handleShow}
                className="d-md-none mt-3 order-2 order-md-1"
              >
              ...
              <Link
                onClick={handleShow}
                className="d-none d-md-block order-2 order-md-1 mb-3 mb-md-0"
              >
                <div className={`${Style.romadyBorder} p-3 rounded-1`}>
                  <i className={`${Style.textMoza} fa-solid fa-plus`}></i>
                </div>
            ...
            <div className="row gy-4 " style={{ direction: "rtl" }}>
              {sessions.map((session) => (
                <div className="col-md-3" key={session._id}>
                  <div
                    className={`${Style.romadyi} d-flex flex-column justify-content-center p-3 align-items-center rounded-2`}
                  >
                    <Link
                      to={`/group/${ID}/${session._id}`}
                    ...
                    <div className="d-flex justify-content-between ">
                      <button
                        ...
                        onClick={() => handleUpdateShow(session)}
                      >
                        Update
                      </button>
                      <button
