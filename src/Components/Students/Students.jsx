...
export default function Students() {
  ...
  const deleteStudent = useCallback(async (id) => {
    try {
      await axios.delete(`https://registration-production-7fcd.up.railway.app/api/v2/students/${id}`);
      queryClient.setQueryData(['students'], (oldData) => 
  ...
  const barCodeClick = useCallback(async (id) => {
    try {
      const response = await axios.get(`https://registration-production-7fcd.up.railway.app/api/v2/students/${id}/qrcode`);
