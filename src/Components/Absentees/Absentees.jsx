...
export default function Absentees() {
  ...
  useEffect(() => {
    axios
      .get("https://registration-production-c3f5.up.railway.app/api/v2/groups")
      .then((response) => {
  ...
  useEffect(() => {
    if (selectedGroup) {
      axios
        .get(
          `https://registration-production-c3f5.up.railway.app/api/v2/sessions/${selectedGroup}`
        )
        .then((response) => {
  ...
  const barCodeClick = async (id) => {
    try {
      const response = await axios.get(
        `https://registration-production-c3f5.up.railway.app/api/v2/students/${id}/qrcode`
      );
  ...
  useEffect(() => {
    if (selectedGroup && selectedSession) {
      axios
        .get(
          `https://registration-production-c3f5.up.railway.app/api/v2/attendance/${selectedGroup}/${selectedSession}`
        )
        .then((response) => {
          console.log(response);
          // Map over absentees and store both student and lastAttendanceRecord in state
          const absenteeData = response.data.absentees.map((absentee) => ({
            student: absentee.student,
            lastAttendanceRecord: absentee.lastAttendanceRecord,
          }));
          setAbsentees(absenteeData); // Store both student and attendance record
        })
        .catch((error) => {
    ...
    <>
      ...
      <div className={` ${Style.serviceHome}`}>
        <div className="container-fluid">
              ...
              <div className="d-flex justify-content-center">
                <div className={`${Style.romady} rounded-3 p-3`}>
                  ...
                  >
                    ...
                    <li className="mx-5 mb-md-0 mb-2">
                      <Link className="text-decoration-none text-white fw-bolder">
                        Attendance
                      </Link>
                    </li>
                  </ul>
          ...
          <div
            className={`${Style.romada} rounded-2 container`}
          ...
          {loading ? (
            ...
            <div className="row gy-5 my-4" style={{ direction: "rtl" }}>
              {filteredStudents.map(({ student, lastAttendanceRecord }) => (
                <div className="col-md-3 text-dark" key={student._id}>
                  ...
                  >
                    <div className="d-flex justify-content-end">
                      <i
                        onClick={() => barCodeClick(student._id)}
                        className="fa-solid fa-qrcode fs-2 cursor-pointer"
                      ></i>
                    </div>
                    <Link
                    ...
                    >
                      ...
                      <h4>
                        الكود:{" "}
                        <span className="h4"> {student.studentCode}</span>
                      </h4>
                      <h4>
                        الكتب:{" "}
                        <span className="h4"> {student.books || "N/A"}</span>
                      </h4>
